...
const Footer = () => {
  return (
   <>
   <Flex bg="white" fontSize="14"  flexDirection={'column'} pt="50px">
    <Container maxW="container.lg">
<Box w="100%" >
  <Flex flexDirection={['column','column','column','row']} w="100%" >
...
<Button bg="blue.500" zIndex={2} rounded="full" fontSize="12"  ml="-10" color="white">Subscribe</Button>
</Flex>
...
const UseFulLinks = ()=>{
  ...
  {name:'Staff',link:'/staff'}, //Terms Of Service
  {name:'Blog',link:'/blog'}] //Privacy Policy
  ...
  return (
     ...
     <Heading mb="2" fontSize={'xl'}>Useful Links</Heading>
    {
      usefullinks.map((val,index)=>(     
        <Link href={val.link}>
        <HStack cursor={'pointer'} _hover={{color:'blue.500'}} color="#aaaaaa" my="2" key={index}>
  <Box color="blue.500">
...
const OurServices = ()=>{
  ...
  return (
    ...
    {
      usefullinks.map((val,index)=>(     
<HStack _hover={{color:'blue.500'}} cursor={'pointer'}  color="#aaaaaa" my="2" key={index}>
<Box  color="blue.500">
  <i className="pi pi-chevron-right"></i>
  </Box>
  <Box>{val.name}</Box>
...
const OutLineCircle = ({ icon }) => {
  return (
      <>
          <Flex color="white" background="blue.300" justifyContent="center" alignItems={'center'}
